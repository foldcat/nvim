...
local function _1_()
  ...
  local colors = {blue = utils.get_highlight("Function").fg, bright_bg = utils.get_highlight("Folded").bg, bright_fg = utils.get_highlight("Folded").fg, cyan = utils.get_highlight("Special").fg, dark_red = utils.get_highlight("DiffDelete").bg, diag_error = utils.get_highlight("DiagnosticError").fg, diag_hint = utils.get_highlight("DiagnosticHint").fg, diag_info = utils.get_highlight("DiagnosticInfo").fg, diag_warn = utils.get_highlight("DiagnosticWarn").fg, git_add = utils.get_highlight("diffAdded").fg, git_change = utils.get_highlight("diffChanged").fg, git_del = utils.get_highlight("diffDeleted").fg, gray = utils.get_highlight("NonText").fg, green = utils.get_highlight("String").fg, orange = utils.get_highlight("Constant").fg, purple = utils.get_highlight("Statement").fg, red = utils.get_highlight("DiagnosticError").fg}
  ...
  File_flags = {{condition = _10_, hl = {fg = "green"}, provider = " [+]"}, {condition = _11_, hl = {fg = "pink"}, provider = " [x]"}}
  local File_name_modifer
  local function _12_()
    if vim.bo.modified then
      return {bold = true, fg = "red", force = true}
    else
      return nil
    end
  end
  ...
  LSPActive = {condition = conditions.lsp_attached, hl = {bold = true, fg = "green"}, provider = _14_, update = {"LspAttach", "LspDetach"}}
  local Git
  ...
  Git = {{hl = {bold = true}, provider = _15_}, {condition = _16_, provider = "("}, {hl = {fg = "purple"}, provider = _17_}, {hl = {fg = "red"}, provider = _18_}, {hl = {fg = "blue"}, provider = _19_}, {condition = _20_, provider = ")"}, condition = conditions.is_git_repo, hl = {fg = "orange"}, init = _21_}
  local Scroll_bar
